...
namespace dso
{
	...
	double PointFrameResidual::linearize(CalibHessian *HCalib)
	{
		...
		float d_d_x, d_d_y;
		{
			...
			// 2019.12.11 yzk
			float depthKinect = target->dDepth[0][(int)Kv * 640 + (int)Ku];
			float blockThs = 2.2; // Scene 5,7,8: 2.7   Scene 6: 2.2
			if (depthKinect > 0 && fabs(1 / new_idepth - depthKinect) > blockThs)
			{
				//std::cout << fabs(1/new_idepth - idepthKinect) << std::endl;
				state_NewState = ResState::OOB;
				return state_energy;
			}
			// 2019.12.11 yzk
