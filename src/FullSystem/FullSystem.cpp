 ...
 *******************************/
	...
	void FullSystem::activatePointsMT()
	{
		...
		for (FrameHessian *host : frameHessians) // go through all active frames
		{
			...
			for (unsigned int i = 0; i < host->immaturePoints.size(); i += 1)
			{
				...
				// 2019.12.06 yzk
				bool canActivate = (ph->lastTraceStatus == IPS_GOOD || ph->lastTraceStatus == IPS_SKIPPED || ph->lastTraceStatus == IPS_BADCONDITION || ph->lastTraceStatus == IPS_OOB) && ph->lastTracePixelInterval < 8 && ph->quality > setting_minTraceQuality && (ph->idepth_max + ph->idepth_min) > 0 && !ph->ck;
	...
	void FullSystem::addActiveFrame(ImageAndExposure *image, ImageAndExposure *depthImage,
									cv::Mat basecolorImg_raw, cv::Mat normalImg_raw,
									cv::Mat metallicImg_raw, cv::Mat roughnessImg_raw,
									int id)
	{
		//[ ***step 1*** ] track线程锁
		if (isLost)
		...
		shell->basecolorImg_raw = basecolorImg_raw.clone();
		shell->metallicImg_raw = metallicImg_raw.clone();
		shell->normalImg_raw = normalImg_raw.clone();
		shell->roughnessImg_raw = roughnessImg_raw.clone();
	...
	void FullSystem::makeKeyFrame(FrameHessian *fh)
	{
		...
		{
			printf("Pose: \n");
			std::cout << frameHessians.back()->shell->camToWorld.matrix() << std::endl;
			cv::Mat img2show = frameHessians.back()->shell->normalImg_raw.clone();
			for (size_t idx = 0; idx < coarseTracker_forNewKF->active_point_u.size(); idx++)
			{
				cv::Point p(coarseTracker_forNewKF->active_point_u[idx], coarseTracker_forNewKF->active_point_v[idx]);
				printf("Pixel at u, v: (%d, %d) \n",
					   p.x, p.y);
				cv::circle(img2show, p, 1, cv::Scalar(0, 0, 255), -1);
			}

			cv::imshow("getNormalImage", img2show);
			fflush(stdout);
		}
	...
	void FullSystem::initializeFromInitializer(FrameHessian *newFrame)
	{
		...
		for (int i = 0; i < coarseInitializer->numPoints[0]; i++)
		{
			...
			if (!std::isfinite(pt->energyTH) || !point->isFromSensor)
			{
				delete pt;
				continue;
			} // 点值无穷大
	...
	void FullSystem::estimateDepthArange(ImmaturePoint *impt, FrameHessian *newFrame)
	{
		...
		/*
	float imageData[wG[0] * hG[0]];
	for(int i = 0; i < wG[0] * hG[0]; i++)
	{
		imageData[i] = newFrame->dI[i][0];
	}

	float refImageData[wG[0] * hG[0]];
	for(int i = 0; i < wG[0] * hG[0]; i++)
	{
		refImageData[i] = refKeyFrame->dI[i][0];
	}

    cv::Mat m(hG[0], wG[0], CV_32FC1);
	memcpy(m.data, imageData, sizeof(float)*wG[0]*hG[0]);
	m.convertTo(m, CV_8UC1, 1.0);
	cv::imshow("newFrame", m);

	cv::Mat m2(hG[0], wG[0], CV_32FC1);
	memcpy(m2.data, refImageData, sizeof(float)*wG[0]*hG[0]);
	m2.convertTo(m2, CV_8UC1, 1.0);
	cv::imshow("refKeyFrame", m2);
	*/
	    ...
	    */
			...
			//if(refKeyFrame->dDepth[0][(int)Pl_camera(1, 0) * wG[0] + (int)Pl_camera(0, 0)] > 0)
			//	continue;
    ...
    */
	...
	float FullSystem::calculateNCC(std::vector<float> vICur, std::vector<float> vIRef)
	{
		...
		//std::cout << "vICur.size() = " << temp1 << std::endl;
