...
namespace dso
{
	...
	void CoarseTracker::makeCoarseDepthL0(std::vector<FrameHessian *> frameHessians)
	{
		...
		active_point_u.clear();
		active_point_v.clear();
		for (FrameHessian *fh : frameHessians)
		{
			for (PointHessian *ph : fh->pointHessians)
			{
				...
				if (ph->lastResiduals[0].first != 0 && ph->lastResiduals[0].second == ResState::IN)
				{
					...
					active_point_u.push_back(u);
					active_point_v.push_back(v);
	...
	// 2019.12.09 yzk
	void CoarseTracker::recordData(int lvl, const SE3 &refToNew)
	{
		int wl = w[lvl];
		int hl = h[lvl];
		...
		for (int i = 0; i < nl; i++)
		{
			...
			int idDepth = Ku + Kv * wl;

			if (idDepth > wl * hl)
				continue;

			float idepthKinect = newFrame->dDepth[lvl][idDepth];

			Vec3f pt2 = RKi * Vec3f(x, y, 1) * (1 / id) + t;
			float idepthCalculate = pt2(2, 0);

			if (!std::isfinite(idepthCalculate) || !std::isfinite(idepthKinect))
				continue;

			if (nl % 10 == 0)
			{
				char str[30];
				sprintf(str, "idepthError-%d.txt", lvl);
				ofstream outfile(string(str), ios::app);
				//cout << "error:" << endl;
				//cout << "idepthCalculate = " << idepthCalculate << endl;
				//cout << "idepthKinect = " << idepthKinect << endl;
				outfile << std::fixed << fabs(idepthCalculate - idepthKinect);
				outfile << endl;
				outfile.close();
			}
		}
	...
	Vec6 CoarseTracker::calcRes(int lvl, const SE3 &refToNew, AffLight aff_g2l, float cutoffTH)
	{
		...
		// block check
		int idDepth = Ku + Kv * wl;
		float depthKinect = newFrame->dDepth[lvl][idDepth];

		Vec3f pt2 = RKi * Vec3f(x, y, 1) * (1/id) + t;
		float depthCalculate = pt2(2, 0);
		...
		{
			case 0:
			    blockThs = 0.5; // Seq1~Seq2:0.5 Seq3~Seq4:1 Seq5~Seq6:1.10 Seq7~Seq8:1.10
			    break;
			case 1:
			    blockThs = 0.5; // Seq1~Seq2:0.5 Seq3~Seq4:0.35 Seq5~Seq6:1.0 Seq7~Seq8:0.73
			    break;
			case 2:
			    blockThs = 0.5; // Seq1~Seq2:0.5 Seq3~Seq4:0.73 Seq5~Seq6:1.0 Seq7~Seq8:1.0
			    break;
			case 3:
			    blockThs = 0.5; // Seq1~Seq2:0.5 Seq3~Seq4:0.43 Seq5~Seq6:1.10 Seq7~Seq8:0.5
			    break;
		}

		if(depthKinect > 0 && fabs(depthCalculate - depthKinect) > blockThs) continue;
		//std::cout << "idepthKinect = " << idepthKinect << "  idepthCalculate = " << idepthCalculate << std::endl;
	    // block check
	    */
			// 2019.12.09 yzk
		...
		IOWrap::Output3DWrapper *wrap)
	{
		...
		// 2019.12.09 yzk
		/*
    for(int lvl2=coarsestLvl; lvl2>=0; lvl2--)
	    recordData(lvl2, refToNew_current);
	*/
		// 2019.12.09 yzk
			...
			for (int y = 3; y < h[lvl] - 3; y++)
				for (int x = 3; x < wl - 3; x++)
				{
					...
					if (bp[0] > 0 || nid >= 3)
					{
						...
						mf.setPixelCirc(x, y, makeJet3B(id)); //: 画 点；
					}
				}
